...
const ChallengeCard = ({item}) => {
    ...
    return (
        <div className="w-full lg:w-96 flex flex-col rounded-xl border bg-cover bg-center">
            <Link to={`/details/${item?._id}`} style={{ backgroundImage: `url(${cardBgImage})` }} className="p-4 relative rounded-lg">
            ...
            <div className="p-4">
               <Link to={`/details/${item?._id}`}><h1 className="text-3xl font-semibold">{item?.contestName}</h1></Link>
